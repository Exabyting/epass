...
use Modules\HRM\Repositories\GCOAppraisalPersonalRequestRepository;
use Modules\HRM\Traits\HistoryTrait;


class GCOAppraisalPersonalRequestService
{
    ...
    public function __construct(
        GCOAppraisalPersonalRequestRepository $gcoAppraisalPersonalRequestRepository,
        AppraisalRequestEvaluationService $gcoAppraisalRequestEvaluationService,
        EmployeeServices $employeeServices
    )
    {
        $this->setActionRepository($gcoAppraisalPersonalRequestRepository);
    ...
    public function saveRequest(array $data)
    {
        return DB::transaction(function () use ($data) {
            ...
            $gcoAppraisalId =$data['gco_appraisal_request_id'] ;
            $gcoAppraisal = GCOAppraisalRequest::find($gcoAppraisalId)->update(['is_save_personal_Info' => 1]);
/*            if (isset($data['job-history'])) {
    ...
    public function requestSubmit(Model $gcoAppraisalRequest)
    {
        ...
        return DB::transaction(function () use ($gcoAppraisalRequest) {
            ...
            $gcoAppraisalId =$gcoAppraisalRequest->gco_appraisal_request_id;

            //$gcoAppraisal = GCOAppraisalRequest::where('id',$gcoAppraisalId)->get();
            $gcoAppraisal = GCOAppraisalRequest::find($gcoAppraisalId)->update(['is_submitted_personal_Info' => 1]);
          ...
          $this->sendNotification(
            ...
            if (env('USE_SYSTEM_MAIL')) {
                $toEmailAddress = env('MAIL_USERNAME', "acr.mopa@inflack.com");
            }
