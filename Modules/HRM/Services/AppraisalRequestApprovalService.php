...
use Modules\HRM\Entities\AppraisalRequestApproval;
use Modules\HRM\Repositories\AppraisalRequestApprovalRepository;

class AppraisalRequestApprovalService
{
    use CrudTrait, NotificationTrait, MailSender;
    private $appraisalRequestApprovalRepository;
    private $roleService;


    public function __construct
    (
        AppraisalRequestApprovalRepository $appraisalRequestApprovalRepository,
        RoleService $roleService
    )
    {
        $this->roleService = $roleService;
        $this->appraisalRequestApprovalRepository = $appraisalRequestApprovalRepository;
        $this->setActionRepository($appraisalRequestApprovalRepository);
    }
    ...
    public function saveRequest(Model $appraisalRequest, array $data)
    {
        return DB::transaction(function () use ($appraisalRequest, $data) {
            ...
            if ($appraisalRequestApproval == null) {
                $appraisalRequestApproval = new AppraisalRequestApproval($data);
