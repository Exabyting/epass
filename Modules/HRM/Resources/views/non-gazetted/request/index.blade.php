                    ...
                    <div class="card-header">
                        ...
                        <div class="heading-elements">
                            ...
                            @can('acr-request-access')
                                <a href="{{ route('ng-appraisal-request.create') }}" class="btn btn-primary btn-lg">
                                    ...
                                    @if(!$request->is_evaluation_submitted)
                                        <tr>
                                            ...
                                            <td>
                                                @cannot('acr-request-access')
                                                    @if(!$request->is_evaluated)
                                                        <a href="{{ route('ng-appraisal-request.first-evaluation', $request->id) }}">{{ $request->requester->first_name }} {{ $request->requester->last_name }}</a>
                                                    @elseif(!$request->is_evaluation_submitted)
                                                        <a href="{{ route('ng-appraisal-request.second-evaluation', $request->id) }}">{{ $request->requester->first_name }} {{ $request->requester->last_name }}</a>
                                                    @endif
                                                @elsecannot($request->requester->user->id == auth()->user()->id)
                                                    @if(!$request->is_submitted)
                                                        <a href="{{ route('ng-appraisal-request.show', $request->id) }}">{{ $request->requester->first_name }} {{ $request->requester->last_name }}</a>
                                                    @else
                                                        <a href="{{ route('ng-appraisal-request.show', $request->id) }}">{{ $request->requester->first_name }} {{ $request->requester->last_name }}</a>
                                                    @endif
                                                @endcannot
                                            ...
                                            <td>
                                                @cannot('acr-request-access')
                                                    @if(!$request->is_evaluated)
                                                        <a href="{{ route('ng-appraisal-request.first-evaluation', $request->id) }}">{{ $request->receiver->first_name }} {{ $request->receiver->last_name }}</a>
                                                    @elseif(!$request->is_evaluation_submitted)
                                                        <a href="{{ route('ng-appraisal-request.second-evaluation', $request->id) }}">{{ $request->receiver->first_name }} {{ $request->receiver->last_name }}</a>
                                                    @endif
                                                @elsecannot($request->requester->user->id == auth()->user()->id)
                                                    @if(!$request->is_submitted)
                                                        <a href="{{ route('ng-appraisal-request.show', $request->id) }}">{{ $request->receiver->first_name }} {{ $request->receiver->last_name }}</a>
                                                    @else
                                                        <a href="{{ route('ng-appraisal-request.show', $request->id) }}">{{ $request->receiver->first_name }} {{ $request->receiver->last_name }}</a>
                                                    @endif
                                                @endcannot
                                                ...
                                                <span aria-labelledby="btnAction"
                                                    ...
                                                    @cannot('acr-request-access')
                                                        @if(!$request->is_evaluated)
                                                            <a class="dropdown-item"
                                                               href="{{ route('ng-appraisal-request.first-evaluation', $request->id) }}"><i
                                                                    class="ft ft-award"></i> Evaluate</a>
                                                        @elseif(!$request->is_evaluation_submitted)
                                                            <a class="dropdown-item"
                                                               href="{{ route('ng-appraisal-request.second-evaluation', $request->id) }}"><i
                                                                    class="ft ft-mail"></i> Submit Evaluation</a>
                                                        @endif
                                                    @endcannot
                                                        <a class="dropdown-item"
                                                           href="{{ route('ng-appraisal-request.show', $request->id) }}"><i
                                                                class="ft-eye"></i> @lang('labels.details')</a>
                                                        @if(!$request->is_submitted)
                                                        <a class="dropdown-item"
                                                           href="{{ route('ng-appraisal-request.edit', $request->id) }}"><i
                                                                class="ft ft-edit"></i> Edit</a>
                                                        <a class="dropdown-item"
                                                           href="{{ route('ng-appraisal-request.submit', $request->id) }}"><i
                                                    ...
                                                    @cannot('acr-request-access')
                                                        @if($request->is_evaluation_submitted && $request->summarizedEvaluation && !$request->is_action_taken)
                                                            @if($request->summarizedEvaluation->receiver->user->id == auth()->user()->id)
                                                                <a class="dropdown-item"
                                                                   href="{{ route('ng-appraisal-request.action', $request->id) }}"><i
                                ...
                                @foreach($allRequests as $request)
                                    @if($request->is_evaluation_submitted)
                                        <tr>
                                            ...
                                            <td>
                                                @if($request->is_evaluation_submitted)
                                                    <a href="{{ route('ng-appraisal-request.second-evaluation-show', $request->id) }}">{{ $request->receiver->first_name }} {{ $request->receiver->last_name }}</a>
                                                @else
                                            ...
                                            <td>
                                                @if($request->is_action_taken)
                                                    <a href="{{ route('ng-appraisal-request.action-approved-view', $request->id) }}">{{ $request->summarizedEvaluation->receiver->first_name }}
                                                        ...
                                                        @if($request->summarizedEvaluation->receiver->id == auth()->user()->id)
                                                            <a class="dropdown-item"
                                                               href="{{ route('ng-appraisal-request.action', $request->id) }}"><i
                                                                    class="ft ft-mail"></i> Take Action</a>
                                                            <a class="dropdown-item" target="_blank"
                                                               href="{{ route('ng-evaluated-acr.print', $request->id) }}"><i
                                                                    class="la la-print"></i> @lang('labels.print')</a>
                                                        @endif
                                                    @endif
                                                </span>
