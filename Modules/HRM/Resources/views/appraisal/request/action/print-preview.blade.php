        ...
        <div class="row">
            <div class="col-12 text-center">
                ...
                <p class="text-left p-lineheight">
                    ...
                    @if($appraisalRequest->action)
                        @lang('hrm::appraisal.action-rating-options.' . $appraisalRequest->action->rating)
                    @endif
                    অধিকিন্তু নিন্মে আমার মন্তব্য যোগ করিতেছিঃ
                </p>
                <p class=" mt-2 p-lineheight text-left mh-100">
                    @if($appraisalRequest->action)
                        {{ $appraisalRequest->action->comment }}
                    @endif
                    {{--                        সম্প্রতি পদোন্নতির হইয়াছে, আরো পদোন্নতি সম্পর্কে মূল্যায়নের সময় হয় নাই এখনো পদোন্নতির যোগ্য নেহেন,--}}
                    {{--                        সম্প্রতি পদোন্নতির হইয়াছে, আরো পদোন্নতি সম্পর্কে মূল্যায়নের সময় হয় নাই এখনো পদোন্নতির যোগ্য নেহেন,--}}
                    {{--                        সম্প্রতি পদোন্নতির হইয়াছে, আরো পদোন্নতি সম্পর্কে মূল্যায়নের সময় হয় নাই এখনো--}}
                </p>
            </div>
        </div>
        <div class="row  align-items-end">
            <div class="col-7">
                <p>তারিখ
                    @if($appraisalRequest->action)
                        {{ en2bn(date('d-m-Y', strtotime($appraisalRequest->action->created_at))) }}
                    @endif
                </p>
            </div>
            <div class="col-5 text-right">
                <p class="text-center d-inline-block">
                    @if($appraisalRequest->action)
                        <img src="{{ url('file/get?filePath=' . $appraisalRequest->action->actor->signature) }}"
                             class="signature-img m-auto" alt="">
                    @endif
                    <br>
