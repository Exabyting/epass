                            ...
                            <table class="table table-striped table-bordered zero-configuration text-center">
                                ...
                                @foreach($actions as $action)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('gco-appraisal-request.action-approved-view', $action->appraisalRequest->id) }}"> {{ $action->appraisalRequest->requester->first_name }} {{ $action->appraisalRequest->requester->last_name }} </a>
                                        </td>
                                        <td>{{$action->appraisalRequest->requester->employee_id}}</td>
                                        <td>{{$action->appraisalRequest->requester->designation->name}}</td>
                                        <td>
                                            <a href="{{ route('gco-appraisal-request.action-approved-view', $action->appraisalRequest->id) }}"> {{ $action->appraisalRequest->receiver->first_name }} {{ $action->appraisalRequest->receiver->last_name }} </a>
                                        </td>
                                        <td>
                                            <a href="{{ route('gco-appraisal-request.action-approved-view', $action->appraisalRequest->id) }}"> {{ $action->actor->first_name }} {{ $action->actor->last_name }} </a>
                                        </td>
                                            ...
                                            <span aria-labelledby="btnAction"
                                                  class="dropdown-menu mt-1 dropdown-menu-right">
                                                    <a class="dropdown-item"
                                                       href="{{ route('gco-appraisal-request.action-approved-view', $action->appraisalRequest->id) }}"><i
                                                            class="ft-eye"></i> @lang('labels.details')</a>
                                                @if(optional(optional($action->appraisalRequest)->approval)->status == 'Completed')
                                                    <div class="dropdown-divider"></div>
                                                    <a class="dropdown-item" target="_blank"
                                                       href="{{ route('gco-acr.print', $action->appraisalRequest->id) }}"><i
                                                ...
                                                @can('system-super-admin')
                                                    @if(optional(optional($action->appraisalRequest)->approval)->status !== 'Completed')
                                                        <div class="dropdown-divider"></div>
                                                        <a class="dropdown-item"
                                                           href="{{ route('gco-appraisal-request-approval.create', $action->appraisalRequest->id) }}"><i
