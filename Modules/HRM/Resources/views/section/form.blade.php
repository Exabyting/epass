        ...
        <div class="col-md-6">
            <div class="form-group">
                <div class="form-group {{ $errors->has('name') ? ' error' : '' }}">
                    ...
                    {{ Form::text('name', null, ['class' => 'form-control', 'placeholder' => 'Section Name', 'required' => 'required', 'data-validation-required-message'=> trans('labels.This field is required')]) }}
                ...
                <div class="form-group">
                    ...
                    <select class="select2 form-control required" name="department_id">
                        <option value=""></option>
                        @if(!is_null($departments))
                            @foreach($departments as $key=>$department)
                                <option value="{{$department->id}}" @if(isset($section) && $section->department_id == $department->id) selected @endif>{{$department->name}}</option>
                            @endforeach
                        @endif
                    </select>
                    @if ($errors->has('section_head_employee_id'))
                        <div class="help-block red">{{ $errors->first('section_head_employee_id') }}</div>
                    @endif
                </div>
{{--                <div class="form-group">--}}
...
{{--                            <option value="{{$key}}" @if(isset($section) && $section->section_head_employee_id == $key) selected @endif>{{$employee}}</option>--}}
{{--                        @endforeach--}}
{{--                    </select>--}}
{{--                    @if ($errors->has('section_head_employee_id'))--}}
